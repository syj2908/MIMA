...
public class CheckStrength {
    ...
    private static String advice = new String();
        ...
        if (null != DICTIONARY && DICTIONARY.length > 0) {// dictionary
            for (int i = 0; i < DICTIONARY.length; i++) {
                if (passwd.equals(DICTIONARY[i]) || DICTIONARY[i].indexOf(passwd) >= 0) {
                    level--;
                    advice = advice.concat("不要包含如passwd的简易密码；\n");
                    break;
                }
            }
        }
    ...
    public static String getAdvice(String passwd) {
        advice = "";
        LEVEL lev;
        lev = getPasswordLevel(passwd);
        if(lev.equals(LEVEL.EXTREMELY_STRONG)||lev.equals(LEVEL.VERY_STRONG)||lev.equals(LEVEL.STRONG))
        {
            advice="密码强度足够高，继续保持！";
        }
        return advice;
